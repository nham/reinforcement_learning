...
use game::Game;
use player::{Player, RandomPlayer, RLPlayer, PX, PO};
...
mod game {
    ...
    impl Game {
        ...
        pub fn get(&self, x: uint, y: uint) -> Result<Cell, &str> {
            if !(x >= 1 && x <= 3 && y >= 1 && y <= 3 ) {
                Err("Out of range")
            } else {
        ...
        pub fn cell_is_avail(&self, x: uint, y: uint) -> bool {
        ...
        pub fn set(&mut self, x: uint, y: uint, c: Cell) -> bool {
            if !(x >= 1 && x <= 3 && y >= 1 && y <= 3 ) {
                false
            } else {
